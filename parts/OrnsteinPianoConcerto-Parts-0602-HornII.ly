...
\score {
	\new Staff \compressMMRests \transpose f c' \keepWithTag #'Part { \markLengthOn \Horn_II_mvmt_I }
	\layout {
	\context {
		\Score
...
\score {
	\new Staff \compressMMRests \transpose f c' \keepWithTag #'Part { \markLengthOn \Horn_II_mvmt_II }
	\layout {
	\context {
		\Score
			rehearsalMarkFormatter = #format-mark-circle-numbers
